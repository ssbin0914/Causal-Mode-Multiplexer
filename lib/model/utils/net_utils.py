...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(20, dets.shape[0])):
        ...
        if score > thresh:
            ...
            cv2.putText(im, '%s: %.2f' % (class_name, score), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), thickness=1)

#             if dets.shape[0] == 1:
#                 final_roi_feat_ = final_roi_feat
#                 roi_feat_ = roi_feat
# ###                relation_matrix_ = relation_matrix
#             else:
#                 final_roi_feat_ = final_roi_feat[i,:,:,:]
#                 roi_feat_ = roi_feat[i,:,:,:]
# ###                relation_matrix_ = relation_matrix[i,:,:]
# ##
#             sio.savemat('./tmp/' + str(img_ind) + '_' + 'final_roi_feat.mat', {'final_roi_feat':final_roi_feat_})
#             sio.savemat('./tmp/' + str(img_ind) + '_' + 'roi_feat.mat', {'roi_feat':roi_feat_})
# ##            sio.savemat('./tmp/' + str(im_ind) + '_' + str(ind) + 'relation_matrix.mat', {'relation_matrix':relation_matrix_})
#           
#          
    return im
...
def _smooth_l1_loss_epi(bbox_pred, bbox_targets, uncer_loc_epi_exp, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
...
def loc_uncertainty_loss(bbox_pred, bbox_targets, var_loc_epi_exp, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    ...
    in_loss_box = in_loss_box * var_loc_epi_exp
...
def _smooth_l1_loss_w_uncertainty(bbox_pred, bbox_targets, loc_uncertain, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
...
def _smooth_l1_loss_penalty(bbox_pred, bbox_targets, loc_uncertain, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    ...
    pred_loc_uncertain = torch.sqrt(torch.clamp(torch.exp(loc_uncertain), min=1e-18))
    in_loss_box = in_loss_box * (1 + pred_loc_uncertain)
