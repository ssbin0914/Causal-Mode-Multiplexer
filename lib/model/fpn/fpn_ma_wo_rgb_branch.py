...
class _FPN(nn.Module):
    ...
    ########################################################
    def fusion(self, f, ir, f_fact=False, ir_fact=False):

        f, ir = self.transform(f, ir, f_fact, ir_fact)

        # z = f * torch.sigmoid(ir)
    ...
    def transform(self, f, ir, f_fact=False, ir_fact=False):
    ...
    def forward(self, im_data, im_info, gt_boxes_ir, gt_boxes_rgb, num_boxes, im_data_ir, session, types='None',
        ...
        #########################################
        te = self.fusion(cls_score, cls_score_ir_branch, f_fact=True, ir_fact=True)  # (1024, 2)
        te_prob = F.softmax(te)
        # te_prob_all = te_prob.view(batch_size, rois_ir.size(1), -1)  # (4, 256, 2)
        te_prob_all = te_prob.view(batch_size, rois_ir.size(1), -1)
        ############################
        ...
        nde = self.fusion(cls_score, cls_score_ir_branch, f_fact=False, ir_fact=True)
        tie = te - bias_detect.unsqueeze(1) * nde
        tie_prob = F.softmax(tie)
        # tie_prob_all = tie_prob.view(batch_size, rois_ir.size(1), -1)
        tie_prob_all = tie_prob.view(batch_size, rois_ir.size(1), -1)
        ############################

        # nde_c = self.fusion(cls_score.clone().detach(), cls_score_ir_branch.clone().detach(), f_fact=False,
        #                     ir_fact=True)  # (1024, 2)
        ...
        ##     nde = self.fusion(cls_score_fused, cls_score_ir, f_fact=False, ir_fact=True)
        ##     tie = te - nde
        ##     tie_prob = F.softmax(tie)
        ##     tie_prob_all = tie_prob.view(batch_size, rois_fused.size(1), -1)
        #########################################
        ...
        if self.training:
            ...
            return rois_ir, tie_prob_all, bbox_pred, roi_pool_feat_rgb, roi_pool_feat_rgb, pooled_feat_rgb, pooled_feat_ir  # var_cls_ir_epi_part, var_cls_rgb_epi_part
