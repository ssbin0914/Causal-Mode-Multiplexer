...
class resnet(_FPN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_layer0_ir = nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu, resnet.maxpool)
    self.RCNN_layer1_ir = nn.Sequential(resnet.layer1)
    self.RCNN_layer2_ir = nn.Sequential(resnet.layer2)
    ...
    self.RCNN_smooth1_ir = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth2_ir = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth3_ir = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)

    # Lateral layers
    self.RCNN_latlayer1_ir = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0)
    ...
    self.RCNN_top_rgb = nn.Sequential(
      nn.Conv2d(256, 1024, kernel_size=cfg.POOLING_SIZE, stride=cfg.POOLING_SIZE, padding=0),
      nn.ReLU(True),
      nn.Conv2d(1024, 1024, kernel_size=1, stride=1, padding=0),
      nn.ReLU(True),
    )
    self.RCNN_top_ir = nn.Sequential(
      nn.Conv2d(256, 1024, kernel_size=cfg.POOLING_SIZE, stride=cfg.POOLING_SIZE, padding=0),
      nn.ReLU(True),
      nn.Conv2d(1024, 1024, kernel_size=1, stride=1, padding=0),
      nn.ReLU(True),
    ...
    self.RCNN_top_ir_branch = nn.Sequential(
      nn.Conv2d(256, 1024, kernel_size=cfg.POOLING_SIZE, stride=cfg.POOLING_SIZE, padding=0),
      nn.ReLU(True),
      nn.Conv2d(1024, 1024, kernel_size=1, stride=1, padding=0),
      nn.ReLU(True),
    ...
    self.RCNN_layer0_ir.apply(set_bn_fix)
    self.RCNN_layer1_ir.apply(set_bn_fix)
  ...
  def train(self, mode=True):
    ...
    if mode:
      ...
      self.RCNN_layer0_ir.eval()
      self.RCNN_layer1_ir.eval()
      self.RCNN_layer2_ir.train()
      self.RCNN_layer3_ir.train()
      self.RCNN_layer4_ir.train()

      self.RCNN_smooth1_ir.train()
      self.RCNN_smooth2_ir.train()
      self.RCNN_smooth3_ir.train()

      self.RCNN_latlayer1_ir.train()
      self.RCNN_latlayer2_ir.train()
      self.RCNN_latlayer3_ir.train()
      ...
      self.RCNN_layer0_ir.apply(set_bn_eval)
      self.RCNN_layer1_ir.apply(set_bn_eval)
      self.RCNN_layer2_ir.apply(set_bn_eval)
  ...
  def _head_to_tail_rgb(self, pool5):
    block5 = self.RCNN_top_rgb(pool5)
    fc7 = block5.mean(3).mean(2)
    return fc7

  def _head_to_tail_ir(self, pool5):
    block5 = self.RCNN_top_ir(pool5)
    fc7 = block5.mean(3).mean(2)
    return fc7
  ...
  def _head_to_tail_ir_branch(self, pool5):
  ...
  def _head_to_tail_ir_dropout(self, pool5):
    ...
    block5 = self.RCNN_top_ir[1](block5)
    block5 = F.dropout(block5, p=0.5, training=True)
    block5 = self.RCNN_top_ir[2](block5)
    block5 = self.RCNN_top_ir[3](block5)

    fc7 = block5.mean(3).mean(2)
    return fc7

  def _head_to_tail_rgb_dropout(self, pool5):
    block5 = self.RCNN_top_rgb[0](pool5)
    block5 = self.RCNN_top_rgb[1](block5)
    block5 = F.dropout(block5, p=0.5, training=True)
    block5 = self.RCNN_top_rgb[2](block5)
    block5 = self.RCNN_top_rgb[3](block5)
