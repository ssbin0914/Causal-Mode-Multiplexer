...
class _FPN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes_ir, gt_boxes_rgb, num_boxes, im_data_ir, session, types='None', UKLoss='ON'):
        UKLoss = 'ON'
...
#        rois_ir = torch.cat((rois_ir_, rois_rgb_), 1)
        ...
        if self.training:
            ...
            ######### Written by JungUk ###########
            real_gt = gt_boxes_ir[0,:,:4]
            real_pred = rois_ir[:,1:]
            total_num = int(gt_boxes_ir[:,:,4].sum())
            index_ = [] #np.zeros(len(real_pred))

            for ii in range(len(real_pred)):
                tmp_ = real_pred[ii,:]
                tmp_val = 0
                for jj in range(total_num):
                    tmp_area = (tmp_[2] - tmp_[0]+1) * (tmp_[3] - tmp_[1]+1)
                    gt_area = (real_gt[jj,2] - real_gt[jj,0]+1) * (real_gt[jj,3]-real_gt[jj,1]+1)

                    over_area_0 = max(real_gt[jj,0], tmp_[0])
                    over_area_1 = max(real_gt[jj,1], tmp_[1])
                    over_area_2 = min(real_gt[jj,2], tmp_[2])
                    over_area_3 = min(real_gt[jj,3], tmp_[3])

                    over_area = max(0,(over_area_2 - over_area_0)) * max(0,(over_area_3 - over_area_1))
                    IoU = over_area / (tmp_area + gt_area - over_area)
                    if IoU > tmp_val:
                        tmp_val = IoU
...
#                cls_score_ir_epi = F.dropout(cls_score_ir_epi, p=0.5, training=True)
...
#                cls_score_rgb_epi = F.dropout(cls_score_rgb_epi, p=0.5, training=True)
                ...
                var_cls_ir_epi_part = (var_cls_ir_epi+1e-10) / (var_cls_ir_epi+var_cls_rgb_epi+1e-10)
                var_cls_rgb_epi_part = (var_cls_rgb_epi+1e-10) / (var_cls_ir_epi+var_cls_rgb_epi+1e-10)

#        print(var_cls_ir_epi_part, var_cls_rgb_epi_part)

################################# To extract pooled feat w/o UDT loss #############################
################################# If not, indent is required ###################################### 
...
#                return rois_ir, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, RCNN_loss_bbox_mix, RCNN_loss_cls_mix, kl_loss, rois_label
                return pooled_feat_rgb, pooled_feat_ir, index_

################################# To extract pooled feat w/o UDT loss #############################
################################# If not, two mix loss should be removed ########################## 
            elif UKLoss == 'OFF':
#                return rois_ir, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, RCNN_loss_bbox_mix, RCNN_loss_cls_mix, rois_label
                return pooled_feat_rgb, pooled_feat_ir, index_
###################################################################################################
###################################################################################################
            ...
            return rois_ir, cls_prob, bbox_pred, roi_pool_feat_rgb, roi_pool_feat_rgb, distance_matrix_rgb, distance_matrix_ir
