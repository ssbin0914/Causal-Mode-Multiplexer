...
def parse_args():
    ...
    parser.add_argument('--use_tfboard', dest='use_tfboard',
                        ...
                        default=False, type=bool)
    parser.add_argument('--test_domain', dest='test_domain',
                        help='which domain to train and test',
...
#                continue
            # RGB -1    I
            filename = imdb.image_path_at(step)[-1]
            data = data_iter.next()
            ...
            FPN.zero_grad()
#            try:
            distance_matrix_rgb, distance_matrix_ir, index = FPN(im_data, im_info, gt_boxes_ir, gt_boxes_rgb, num_boxes, im_data_ir, args.session, args.types, args.UKLoss) #args.session, args.test_domain)
#            except:
#                continue
            distance_matrix_rgb = distance_matrix_rgb.data.cpu().numpy()
            distance_matrix_ir = distance_matrix_ir.data.cpu().numpy()
#            print(filename) 
            sio.savemat('./Feature_Proposed/' + str(filename.split('/')[-1].split('.')[0]) + '.mat', {'feat_rgb': distance_matrix_rgb, 'feat_ir': distance_matrix_ir,'index':index})

#            roi_labels = roi_labels.transpose(0, 1)

#            sio.savemat('./features/%s/%s.mat' %(args.test_domain, filename.split('/')[-1].split('.')[0]), {'roi_labels': roi_labels.data.cpu().numpy(),'%s_feat' %(args.test_domain):pooled_feat.data.cpu().numpy()})

        

        print("Finish Feature Extractions")
        assert 1==0

#            print(step)
#            if step == 0:
#                concat_roi_labels = roi_labels.data.cpu().numpy()
#                concat_pooled_feat = pooled_feat.data.cpu().numpy()
#            else:
#                concat_roi_labels = np.concatenate((concat_roi_labels, roi_labels.data.cpu().numpy()), axis=0)
#                concat_pooled_feat = np.concatenate((concat_pooled_feat, pooled_feat.data.cpu().numpy()), axis=0)

#            if step == 199:
#                print(concat_roi_labels.shape, concat_pooled_feat.shape)
#                sio.savemat('./features/rgb_night.mat', {'rgb_night_labels':concat_roi_labels, 'rgb_night_feat':concat_pooled_feat})
#                print("Finish extracting RGB features in NIGHT")
#                assert 1==0
